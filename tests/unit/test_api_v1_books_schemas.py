# pylint: disable=no-member
# Copyright © 2026 SpendKey. All Rights Reserved.
"""Books schema tests."""
import typing as t

import pydantic
import pytest

from app.api.v1.books import schemas


def test_book() -> None:
    """Should be a Pydantic model."""
    assert issubclass(schemas.Book, pydantic.BaseModel)  # type: ignore


@pytest.mark.parametrize(
    "attr",
    [
        "id",
        "name",
        "description",
        "isbn",
        "tags",
        "price",
        "author_id",
        "publisher_id",
        "ai_summary",
        "created_at",
        "updated_at",
    ],
)
def test_book_attributes(attr: str) -> None:
    """Should have the expected attributes.

    Args:
        attr: Expected schema attribute.
    """
    properties: dict[str, t.Any] = schemas.Book.schema().get("properties", {})
    assert attr in properties


def test_book_list() -> None:
    """Should be a Pydantic model."""
    assert issubclass(schemas.BookList, pydantic.BaseModel)


@pytest.mark.parametrize(
    "attr",
    [
        "total",
        "data",
    ],
)
def test_book_list_attributes(attr: str) -> None:
    """Should have the expected attributes.

    Args:
        attr: Expected schema attribute.
    """
    properties: dict[str, t.Any] = schemas.BookList.schema().get(
        "properties", {}
    )
    assert attr in properties
